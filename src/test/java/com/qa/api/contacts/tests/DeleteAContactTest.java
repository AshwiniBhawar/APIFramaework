...
public class DeleteAContactTest extends BaseTest{
	...
	public void createAContact() {
		...
		// 3. Delete - delete the contact using same contactId
		Response deleteResponse = restClient.delete(BASE_URL_CONTACTS, CONTACTS_ENDPOINT + "/" + contactId, null,
				null, AuthType.BEARER_TOKEN, ContentType.JSON);
		Assert.assertEquals(deleteResponse.statusCode(), 200);
		Assert.assertTrue(deleteResponse.statusLine().contains("OK"));
		Assert.assertTrue(deleteResponse.getBody().asString().contains(AppConstants.CONTACT_DELETE_MSG));
		

		System.out.println("===================================================================================================");

		// 4.Get - get the deleted contact using same contactId
